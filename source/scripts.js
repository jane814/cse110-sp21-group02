...
function init () {
  ...
  for (let i = 0; i < predefinedQuestions.length; i++) {
    const question = document.createElement('option');
...
function renderHistory () {
  ...
  history.innerHTML = '';
  for (const index in readings) {
    const historyObj = historyList[index];
    const historyItem = document.createElement('div');
    historyItem.classList.add('history-item');
    historyItem.id = `history-item-${historyObj.id}`;
    const historyItemImg = document.createElement('img');
    ...
    const historyItemImg2 = document.createElement('img');
    ...
    const historyItemImg3 = document.createElement('img');
    ...
    const historyItemBtn = document.createElement('button');
    ...
    const historyItemBtn2 = document.createElement('button');
 ...
 */
function renameReading (name, id) { // eslint-disable-line no-unused-vars
  const readings = getReadings();
...
function deleteReading (id) {
  const readings = getReadings();
  const result = readings.filter(reading => reading.id != id);
  saveReadings(result);
}
 ...
 */
function deleteAllReadings () { // eslint-disable-line no-unused-vars
...
function generateHandler () {
  const question = getUserInputText();
...
function displayReading () {
  const imageLeft = document.getElementById('display-img-left');
  const imageMid = document.getElementById('display-img-mid');
  const imageRight = document.getElementById('display-img-right');
...
function displayHomeScreen() {
  ...
  let itemsToHide = document.getElementsByClassName('to-show');
...
function retryHandler () {
  const question = currentReading.userInput;
  const reading = generateReading(question);
  currentReading = reading;
  displayReading();
}
 ...
 */
function generateReadingLink (reading) { // eslint-disable-line no-unused-vars
  // TODO: implement (v2)
  console.log(reading);
  return '';
}
